...
/**
 * Static lookup of all 148 CSS named colors plus {@code transparent}.
 *
 * <p>Color values are sourced from the W3C CSS Color Level 4 specification:
 * <a href="https://www.w3.org/TR/css-color-4/#named-colors">Named Colors</a>.</p>
 *
 * <p>Lookup is case-insensitive. The returned {@link Color} objects use sRGB color space.</p>
 */
public final class NamedCssColors {

    private static final Map<String, Color> COLORS;

    static {
        Map<String, Color> map = new LinkedHashMap<>(200);

        // CSS Color Level 4 named colors (148 entries + transparent)
        ...
        map.put("rebeccapurple", new Color(102, 51, 153));
        ...
        // Special: transparent (rgba 0,0,0,0)
        map.put("transparent", new Color(0, 0, 0, 0));

        COLORS = Collections.unmodifiableMap(map);
    }

    private NamedCssColors() {
        // utility class
    }

    /**
     * Resolves a CSS named color to a {@link Color} instance.
     *
     * <p>Lookup is case-insensitive. Returns {@link Optional#empty()} if the
     * name is not a recognized CSS named color.</p>
     *
     * @param name the CSS color name (e.g., "red", "aliceblue", "transparent")
     * @return an Optional containing the Color if found, or empty
     */
    public static Optional<Color> resolve(String name) {
        if (name == null || name.isEmpty()) {
            return Optional.empty();
        }
        return Optional.ofNullable(COLORS.get(name.toLowerCase(Locale.ROOT)));
    }

    /**
     * Returns the total number of named colors in the lookup table.
     *
     * @return the count of named colors (148 standard + transparent = 149)
     */
